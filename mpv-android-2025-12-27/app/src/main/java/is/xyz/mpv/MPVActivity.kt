...
class MPVActivity : AppCompatActivity(), MPVLib.EventObserver, TouchGesturesObserver {
    ...
    override fun onCreate(icicle: Bundle?) {
        ...
        val savedPos = VideoHistory.getPosition(applicationContext, filepath)
        if (savedPos > 3L) {
            // Seek after file load — mpv fires a file-loaded event we can hook,
            // but simplest is a short delayed seek
            binding.player.postDelayed({
                MPVLib.command(arrayOf("seek", savedPos.toString(), "absolute"))
            }, 500)
        }
    ...
    private fun savePosition() {
        // Always save to VideoHistory for resume, regardless of setting
        val pos = player.timePos
        val filePath = MPVLib.getPropertyString("path")
        if (pos != null && filePath != null) {
            val isEof = MPVLib.getPropertyBoolean("eof-reached") ?: false
            if (!isEof && pos > 3.0) {
                VideoHistory.savePosition(applicationContext, filePath, pos.toLong())
            } else if (isEof) {
                // Finished — clear saved position so thumbnail shows no progress bar
                VideoHistory.clearPosition(applicationContext, filePath)
            }
        }

        // Original mpv watch-later logic (pref-gated)
